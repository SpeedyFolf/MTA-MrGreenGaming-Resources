	...
	if dataReceived and firstSpawned then
		...
		if c > race["maxCP"] then
			...
			for i = getElementData(localPlayer, "race.checkpoint"), 26 do
				...
				setElementPosition(getPedOccupiedVehicle(localPlayer), 2146.7002, 5000.0000, 100*c)
...
addEventHandler("onClientColShapeHit", root, function(element, dim)
	if element == localPlayer and not isLocalPlayerSpectating() and source == markerCol and not isTimer(testTimer) then
		testTimer = setTimer(function() end, 1000, 1)
		...
		setElementPosition(getPedOccupiedVehicle(localPlayer), 2146.7002, 5000.0000, 100*c)
		setElementPosition(getPedOccupiedVehicle(localPlayer), x, y, z)
		setElementVelocity(getPedOccupiedVehicle(localPlayer), vx, vy, vz)
